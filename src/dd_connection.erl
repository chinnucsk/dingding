...
-export([send_msg/2, send_msg/5, reply/3, send_raw/2, handle/2, handle_numeric_reply/3, add_module/2, remove_module/2, show_modules/1]).
...
remove_module_from_channel(Pid, ModuleName, Channel) ->
    gen_sever:cast(Pid, {remove_module_from_channel, ModuleName, Channel}).
...
handle(#ircmsg{prefix=_P, command= <<"PONG">>, arguments=_A, tail=_T}=_Msg, #state{}=State) ->
    ...
    {ok, State};
handle(#ircmsg{prefix=__P, command = <<"001">>, arguments=__A, tail=__T}=_Msg, #state{serverconfig=#serverconfig{modules=_Modules, channels=_Channels}}=State) ->
    join_channels(State);

...
load_config() ->
    %% check if we have the old version of the config still.
    %% old version means: the old file is there but the new file isn't.
    PrivDir = code:lib_dir(dd, priv),    
    {ok, CfgTerms, _} = file:path_consult([PrivDir], "dd.cfg"),
